...
def safe_merge_dfs(df_main, df_sub, interval):
    if logging.getLogger('yfinance').level == logging.DEBUG:
        print(f"safe_merge_dfs({interval})")
    if df_sub.shape[0] == 0:
    ...
    if f_outOfRange.any():
        ...
        get_yf_logger().info(f'Discarding these {data_col} events:' + '\n' + str(df_sub[f_outOfRange]))
    ...
    if logging.getLogger('yfinance').level == logging.DEBUG:
        for i in range(len(indices)):
            idx = indices[i]
            print(f'{df_sub.index[i]} mapped to range [{df_main.index[idx]} {df_main.index[idx+1]})')
